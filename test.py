...
def randomMatrix(matrixSize, minValue, maxValue):
    matrix = []
...
def builtDataset(datasetSize,matrixSize,minValue, maxValue):
    inputs = []
    outputs = []
    for i in range(datasetSize):
        inputM = randomMatrix(matrixSize,minValue, maxValue)
...
inputs,outputs = builtDataset(datasetSize,matrixSize, minValue,maxValue)
...
history = model.fit(X_train, Y_train, batch_size=100, shuffle=True, epochs=epochs, validation_split=0.1)
...
inputs,outputs = builtDataset(datasetSize,matrixSize, minValue,maxValue)
_, X, _, Y = train_test_split(inputs, outputs, test_size=0.1, random_state=seed)

print(Y[0])
yhat = model.predict(np.array([X[0]]))
print(yhat)
